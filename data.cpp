...
char* get_json_live_temperature()
{
	...
	json_object_object_add(json, "temperature", json_object_new_double((double)meteo_data_temperature/100));
...
char* get_json_live_humidite()
{
    ...
    // Ajouter les données
    json_object_object_add(json, "timestamp", json_object_new_int(meteo_data_time));
    json_object_object_add(json, "humidite", json_object_new_double((double)meteo_data_humidite / 100));
...
char* get_json_live_pression()
{
    ...
    // Ajouter les données
    json_object_object_add(json, "timestamp", json_object_new_int(meteo_data_time));
    json_object_object_add(json, "pression", json_object_new_double((double)meteo_data_pression / 100));
...
char* get_json_data_loc(int timestampMin, int timestampMax, int nbPoint, char val[5])
{
	...
	if (strcmp(val, "temp") == 0)
	{
        ...
        {
            simple_data(json, (double)meteo_data_temperature / 100);
        }
        else
        {
            if (nbPoint > 1)
            {
                ...
                complex_data(json_table, resultat, nbPoint, ligne);
